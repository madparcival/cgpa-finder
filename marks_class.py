    ...
    def markInsert(semester,marksDict):
        ...
        for i in range(1,partsCount+1):
            ...
            for j in range(1,numberOfSubjects+1):
                ...
                credits=int(input(f"Enter the credits{j}:"))
                marksDict[f"semester_{semester}"][f"part_{i}"][f"mark_{j}"]=mark
                marksDict[f"semester_{semester}"][f"part_{i}"][f"credit_{j}"]=credits
            marksDict[f"semester_{semester}"][f"part_{i}"]['grade']=0
